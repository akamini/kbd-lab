...
- カラースキーム:
  ...
  - アクセントカラー: #50c9ea（薄い青）
  - テキストカラー: 白、薄いグレー
  - KBD-LAB ロゴのデザインと統一感を持たせるため、全体的なデザインテーマは青系統を基調とする。
...
- 認証: Supabase Auth を利用。
- ログイン方法: Github アカウントによるログインのみを許可。Google アカウントによるログインは将来実装予定。メール/パスワードによるログインは許可しない。
- 権限管理:
  - 一般ユーザー: プロダクトの閲覧、検索、お気に入り登録。
  - 投稿ユーザー (ログインユーザー): プロダクトの登録、編集、削除。
  - 管理者: サイト全体の管理、不適切コンテンツの削除など（別途定義）。
- Row Level Security (RLS): Supabase の機能を活用し、ユーザーごとのデータアクセス制御を実装。
...
- 投稿者は以下の情報を登録可能:
  ...
  - 販売ページの URL
  - 任意のタグ（複数設定可能）
- 投稿者は自身が登録したプロダクトを編集・削除可能。

#### 4.4. 検索機能

- ユーザーはプロダクトをタグで検索できる。
- フリーワード検索（プロダクト名、説明文など）。

#### 4.5. 収益化機能

- サイト運営者は、主に「工具・便利工具一覧ページ」に Amazon アソシエイトや楽天アフィリエイトなどのアフィリエイトリンクを掲載できる。
...
- UI ライブラリ: shadcn/ui
- データベース: Supabase
- クライアント: Supabase JavaScript Client
- 認証: Supabase Auth
...
- レスポンシブデザインに対応し、スマートフォンやタブレットでも快適に閲覧できること。

#### 5.3. パフォーマンス

- ページの表示速度が速いこと。
- 大量のプロダクトデータが登録された場合でも、検索や表示、ランキング集計が遅延しないこと。

#### 5.4. セキュリティ

- Supabase のセキュリティ機能（RLS、認証など）を活用し、安全なシステムを構築する。
- XSS、CSRF などの一般的なウェブ脆弱性対策を施す。
- 個人情報の取り扱いには細心の注意を払う（プライバシーポリシーの策定・表示）。
...
- 単体テスト: コンポーネントやユーティリティ関数の単体テストを実施。
- 統合テスト: 複数のコンポーネントや API との連携を含む統合テストを実施。
- E2E テスト: ユーザーの実際の操作を模したエンドツーエンドテストを実施。
- アクセシビリティテスト: WAI-ARIA ガイドラインに準拠しているかテスト。
- パフォーマンステスト: ページ読み込み速度やレスポンス時間のテスト。
- セキュリティテスト: 脆弱性スキャンや認証・認可のテスト。

### 6. 成果物（想定）

- ウェブサイト一式（ソースコード、設定ファイルなど）
- データベーススキーマ定義 (Supabase 形式)
- テスト結果報告書
- （必要に応じて）簡単な運用マニュアル
