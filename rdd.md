...
- ログイン方法: Google アカウントによるログインを主とし、メール/パスワードも許可。
- 権限管理:
  - 一般ユーザー: プロダクトの閲覧、検索、お気に入り登録。
  - 投稿ユーザー (ログインユーザー): プロダクトの登録、編集、削除。
  - 管理者: サイト全体の管理、不適切コンテンツの削除など（別途定義）。
- Row Level Security (RLS): Supabaseの機能を活用し、ユーザーごとのデータアクセス制御を実装。
...
#### 5.4. セキュリティ
- Supabaseのセキュリティ機能（RLS、認証など）を活用し、安全なシステムを構築する。
...
- 統合テスト: 複数のコンポーネントやAPIとの連携を含む統合テストを実施。
- E2Eテスト: ユーザーの実際の操作を模したエンドツーエンドテストを実施。
- アクセシビリティテスト: WAI-ARIAガイドラインに準拠しているかテスト。
